...
impl ProsocialProfile {
    ...
    ///   3. Interdependence emerges when people both give and receive
    fn behavior_output(&self) -> BehaviorOutput {
        ...
        // (e.g., lying to protect someone — pro-social deception)
        let likely_truthful = self.honest_by_default || (effective_empathy > 65.0);

        // Interdependence: mutual reliance creates bonds.
        // Highest when people both give AND have needs met enough to give.
        let interdependence =
            effective_empathy * if likely_to_help { 1.0 } else { 0.4 } * available_capacity;
...
fn main() {
    ...
    let avg_eff = people
        .iter()
        .map(|p| p.behavior_output().effective_empathy)
        .sum::<f32>()
        / people.len() as f32;
    ...
    let helpers = people
        .iter()
        .filter(|p| p.behavior_output().likely_to_help)
        .count();
    ...
    println!("  • Interdependence peaks when people both give AND receive.");
    println!("    Isolation (no needs met) breaks the cycle.");
    println!();
}
